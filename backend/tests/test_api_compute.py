...
class TestComparison:
    def test_compare_properties(self, client):
        p1 = client.post(
            "/api/properties",
            json={"name": "P1", "listing_price": 400000, "annual_taxes": 5000},
        ).json()["id"]
        p2 = client.post(
            "/api/properties",
            json={"name": "P2", "listing_price": 300000, "annual_taxes": 4000},
        ).json()["id"]
...
class TestComputeWithTier1Fields:
    def test_results_include_rental_delay(self, client):
        ...
        resp = client.post(
            "/api/properties",
            json={"name": "Delay Test", "listing_price": 400000, "annual_taxes": 6000},
        )
        pid = resp.json()["id"]
        # Update assumptions with rental delay
        client.put(
            f"/api/properties/{pid}/assumptions", json={"rental_delay_months": 2}
        )
        # Create scenario
        client.post(
            f"/api/properties/{pid}/scenarios",
    ...
    def test_dscr_warning_when_low(self, client):
        ...
        resp = client.post(
            "/api/properties",
            json={"name": "DSCR Test", "listing_price": 600000, "annual_taxes": 10000},
        )
        pid = resp.json()["id"]
        client.put(
            f"/api/properties/{pid}/assumptions",
            json={"avg_nightly_rate": 100, "occupancy_pct": 40},
        )
        client.post(
            f"/api/properties/{pid}/scenarios",
            json={
                ...
                "down_payment_pct": 25,
                "down_payment_amt": 150000,
                "interest_rate": 8.0,
                "loan_term_years": 30,
                "closing_cost_pct": 3,
                "closing_cost_amt": 18000,
            },
        )
        resp = client.get(f"/api/properties/{pid}/results")
        assert resp.status_code == 200
    ...
    def test_gross_receipts_tax_in_expenses(self, client):
        resp = client.post(
            "/api/properties",
            json={"name": "Burlington", "listing_price": 400000, "annual_taxes": 6000},
        )
        pid = resp.json()["id"]
        client.put(
            f"/api/properties/{pid}/assumptions",
            json={"local_gross_receipts_tax_pct": 9.0},
        )
        client.post(
            f"/api/properties/{pid}/scenarios",
            json={
    ...
    def test_tax_impact_display(self, client):
        resp = client.post(
            "/api/properties",
            json={"name": "VT Tax", "listing_price": 400000, "annual_taxes": 6000},
        )
        pid = resp.json()["id"]
