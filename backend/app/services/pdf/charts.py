...
def render_projection_chart(projections: list[dict]) -> bytes:
    ...
    # Cashflow bars
    bar_colors = [COLORS["positive"] if cf >= 0 else COLORS["negative"] for cf in cashflows]
    ax1.bar(years, cashflows, width=0.5, color=bar_colors, alpha=0.85, label="Annual Cashflow", zorder=3)
    ...
    ax2 = ax1.twinx()
    ax2.plot(years, equities, color=COLORS["primary"], marker="o", markersize=5, linewidth=2, label="Equity", zorder=4)
    ax2.plot(years, prop_values, color=COLORS["secondary"], marker="s", markersize=4, linewidth=1.5, linestyle="--", label="Property Value", zorder=4)
    ...
    ax1.legend(lines1 + lines2, labels1 + labels2, fontsize=7, loc="upper left", framealpha=0.9)
...
def render_sensitivity_chart(occupancy_sweep: list[dict], rate_sweep: list[dict]) -> bytes:
    ...
    ax1.plot(occ_pcts, occ_cfs, color=COLORS["primary"], linewidth=2, marker="o", markersize=3)
    ax1.axhline(y=0, color=COLORS["negative"], linewidth=0.8, linestyle="--", alpha=0.6)
    ax1.fill_between(occ_pcts, occ_cfs, 0, where=[cf >= 0 for cf in occ_cfs], alpha=0.1, color=COLORS["positive"])
    ax1.fill_between(occ_pcts, occ_cfs, 0, where=[cf < 0 for cf in occ_cfs], alpha=0.1, color=COLORS["negative"])
    ax1.set_xlabel("Occupancy %", fontsize=8, color="#475569")
    ax1.set_ylabel("Monthly Cashflow", fontsize=8, color="#475569")
    ax1.set_title("Occupancy Sensitivity", fontsize=9, fontweight="bold", color="#1e293b")
    ax1.yaxis.set_major_formatter(mticker.FuncFormatter(_currency_formatter))
    ...
    ax2.plot(rates, rate_cfs, color=COLORS["secondary"], linewidth=2, marker="o", markersize=3)
    ax2.axhline(y=0, color=COLORS["negative"], linewidth=0.8, linestyle="--", alpha=0.6)
    ax2.fill_between(rates, rate_cfs, 0, where=[cf >= 0 for cf in rate_cfs], alpha=0.1, color=COLORS["positive"])
    ax2.fill_between(rates, rate_cfs, 0, where=[cf < 0 for cf in rate_cfs], alpha=0.1, color=COLORS["negative"])
...
def render_ltr_sensitivity_chart(vacancy_sweep: list[dict], rent_sweep: list[dict]) -> bytes:
    ...
    ax1.plot(vac_pcts, vac_cfs, color=COLORS["primary"], linewidth=2, marker="o", markersize=3)
    ax1.axhline(y=0, color=COLORS["negative"], linewidth=0.8, linestyle="--", alpha=0.6)
    ax1.fill_between(vac_pcts, vac_cfs, 0, where=[cf >= 0 for cf in vac_cfs], alpha=0.1, color=COLORS["positive"])
    ax1.fill_between(vac_pcts, vac_cfs, 0, where=[cf < 0 for cf in vac_cfs], alpha=0.1, color=COLORS["negative"])
    ...
    ax2.plot(rents, rent_cfs, color=COLORS["secondary"], linewidth=2, marker="o", markersize=3)
    ax2.axhline(y=0, color=COLORS["negative"], linewidth=0.8, linestyle="--", alpha=0.6)
    ax2.fill_between(rents, rent_cfs, 0, where=[cf >= 0 for cf in rent_cfs], alpha=0.1, color=COLORS["positive"])
    ax2.fill_between(rents, rent_cfs, 0, where=[cf < 0 for cf in rent_cfs], alpha=0.1, color=COLORS["negative"])
